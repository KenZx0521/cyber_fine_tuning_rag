...
- ✅ **加權已算**（2026-05-27）：`scripts/compute_sampler_weights.py`（α=0.5 cap=5）→ 調整後 Fenrir 90% → 60%、reasoning → 23%、AttackQA → 10%、Primus → 6%；詳見〈三、行動與狀態〉與 memory `cyber-ft-train-weighting`。
...
| 3 | 重算來源加權 | ✅ 已算（`scripts/compute_sampler_weights.py`，α=0.5 cap=5）；**套用屬訓練階段** |
| 4 | primus/general 離題去留 | ✅ 已剔（`--drop-offtopic`，296 → 83） |

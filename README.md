...
```bash
uv sync          # 安裝依賴（torch / transformers / peft / trl / bitsandbytes / tensorboard …）
```
...
`--output-dir <路徑>`（自訂輸出位置）。

### 4. 監控

```bash
uv run tensorboard --logdir outputs/qlora-cyber/run
```

輸出位於 `outputs/qlora-cyber/run/`：訓練中的 checkpoints + 結束時的最終 adapter
（`outputs/qlora-cyber/run/final-adapter`）。
